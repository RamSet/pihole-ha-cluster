...
- **Slow peer logins:** Pi-hole hashes the app password deliberately slowly, so on a small or busy host a peer login can take many seconds. If the log reports a login timeout, raise `PIHOLE_HA_AUTH_TIMEOUT` (default 10s) rather than assuming the hardware is inadequate. `PIHOLE_HA_AUTH_RETRY_SEC` (default 60s) controls how long a failed login waits before being retried. Editing `nodes.conf` inside the volume works too, but `PIHOLE_HA_FORCE_CONFIG=true` regenerates it — the env vars are the durable place.
